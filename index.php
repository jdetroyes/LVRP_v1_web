            ...
            if(isset($_GET['do']))
            {
                switch($_GET['do'])
                {
					...
					case 'register':
						...
						break;
					case 'rules':
						$filerules = file_get_contents("functions/help_Rules.php");
                        echo char($filerules);
						break;
					case 'deff':
						$filedef = file_get_contents("functions/help_Deff.php");
                        echo char($filedef);
						break;
					case 'vote':
						$rStats = mysql_query("SELECT * FROM `lvrp_users` WHERE `Name`='".$_SESSION['login']."'");
						$dStats = mysql_fetch_array($rStats);
						$time1 = date("h:i:s",$dStats['Vote_Time']);
						...
						else
						{
							if(isset($_POST['login']) and $_POST['login'] != null)
							{
								if(isset($_POST['password']) and $_POST['password'] != null)
								{
									...
									{
										if($dLogin['Pass'] == sha1($_POST['password']))
										{
